...
of log of maize yield of the left-out observation. Then we calculated the absolute value of the difference between the predicted and real value of the left-out observation for each of the $n$ models. In the next step, we averaged the $n$ absolute values of the differences between predicted and real values to obtain the mean absolute prediction error (MAPE). Finally, we scaled the MAPE values by dividing them by the interquartile range (IQR) of the sample obtained as a difference between the third and first quartiles.


Besides obtaining the MAPE of our preferred specifications, we calculated MAPE of baseline models specified as mixed effects models with the set of fixed effects including two measures, in particular cumulative seasonal precipitation and seasonal average temperature. The only random effects of the baseline models are random intercepts as in our preferred specification and the baseline models have the same error structure ARMA($1$,$1$) as our main models. For the sake of comparison, we further calculated MAPE of naive forecasts defined as the last period's actual values of each county.

The scaled MAPE of our preferred specification and the two baseline predictions can be found in table~\ref{CVdiv}. All three predictions were obtained for the sample of all counties and also for the models based on the subsamples of ASAL and non-ASAL counties. The values of MAPE not divided by IQR can be found in table~\ref{CV} in the appendix.
...
According to MAPE, our preferred specifications perform better than the baseline mixed effects models and considerably better than the naive forecasts for all three samples (table~\ref{CVdiv}). For all three prediction methods, the smallest values of MAPE were achieved for the sample of all counties while the least precise forecasts with regard to MAPE were obtained for the subsample of ASAL counties (table~\ref{CVdiv}).
