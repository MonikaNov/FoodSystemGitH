...
In this study, we analyzed the relationship between maize yields and climate. Our dataset consisted of an yearly panel of $47$ counties of Kenya describing the period of $1981-2017$. 
	We acquired the county level yield data from the Famine Early Warning Systems Network (FEWS NET). As for the weather data, we utilized $0.25^\circ$ resolution daily precipitation and temperature gridded data. The precipitation data come the Climate Hazards Group InfraRed Precipitation with Station data (CHIRPS) while the temperature data are available at the website of the Berkeley Earth. 
	
	
	In order to obtain a conformable dataset of yield, precipitation and temperature (that is all data should have same frequency and spatial resolution), the gridded weather data were first averaged over the counties of Kenya so that we obtained daily time series of each county. In the next step, we aggregated the resulting county level time series temporally to obtain yearly time series corresponding to the frequency of the yield data. We only considered weather data during the rainy seasons which we defined as follows, using the classification of...
