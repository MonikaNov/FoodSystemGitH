...
To find the most suitable set of predictors, we adopted a step-down model building approach using the Satterthwaite's method to determine the p-values of the individual t-test \cite{lmerTest}. That is, we started by including of all possible weather measures in both fixed effects and random effects and we consequently removed the insignificant ones until we got model with only significant fixed and random effects. For both fixed and random effects we considered the level of significance $\alpha=0.05$. 
