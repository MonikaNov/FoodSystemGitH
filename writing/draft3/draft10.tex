...
To find the most suitable set of predictors, we adapted a step-down model building approach using the Satterthwaite's method to determine the p-values of the individual t-tests \cite{lmerTest}. To be more specific, we started with a model which included the complete set of our weather measures in both fixed effects and random effects. In the following steps, we were sequentially removing the insignificant variables until we were left with a model with only significant predictors in fixed and random effects. For both fixed and random effects we considered the level of significance $\alpha=0.05$. 

According to the conditional Lagrange multiplier (LM) test for serial correlation in the idiosyncratic component of the errors (developed by Baltagi \& Li 1991 and Baltagi \& Li 1995), the errors exhibited a within group autcorrelation structure in our models (the p-value of the LM test was $9.48\times10^{-14}$). To investigate the autocorrelation structure of the errors, we estimated several variants of our preferred specification with the autocorrelation structure of a form ARMA($p$,$q$). In particular, we estimated all varieties of our model with ARMA($p$,$q$) errors such that $p\leq2$ and $q\leq2$. Comparing the AIC criteria and using the corresponding likelihood ratio statistics, we found that the most appropriate correlation structure of the errors was ARMA($1$,$1$). The AIC criterion of the model with ARMA($1$,$1$) error structure was $2122.2$ while of the model with ARMA($1$,$0$) errors was $2129.2$. The p-value of the likelihood ratio test of comparison of the with ARMA($1$,$1$) errors and the one with ARMA($1$,$0$) errors was smaller than $1 \times10^{-4}$, hence the ARMA($1$,$1$) errors structure is preferable. The AIC criteria and likelihood ratio test statics of all models with ARMA($p$,$q$) errors can be found in table~\ref{ARMA} in the appendix.
...
\begin{tablenotes}
  \begin{footnotesize}
  \item[a] Likelihood ratio test of comparison with the ARMA(${1}$,${1}$) error structure model. ARMA(${1}$,${1}$) error structure seems to be the most suitable as all lower-order correlation structure models are rejected against ARMA(${1}$,${1}$) while ARMA(${1}$,${1}$) is not rejected against the higher order structures.
\singlespacing
  \end{footnotesize}
\end{tablenotes}
  \end{threeparttable} 
