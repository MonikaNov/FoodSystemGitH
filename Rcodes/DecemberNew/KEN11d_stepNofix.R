...
fullishML<-lme(log(Yield0)~SeasPr+AvgTemp+I(SeasPr^2)+ Prec2m+CVPrec+Spell+Spell4 +MaxP
            ...
            data=ScaledTS,na.action=na.exclude); summary(fullishML); exp(summary(fullishML)$coef[[1]])

    CaryML_stepAIC<-stepAIC(fullishML);   summary(CaryML_stepAIC);CaryML_stepAIC;CaryML_stepAIC$anova
...
fullish_lmer<-lmer(log(Yield0)~SeasPr+AvgTemp+I(SeasPr^2)+ Prec2m+CVPrec+Spell+Spell4 +MaxP
          ...
          Cary_stepAIC<-stepAIC(fullish_lmer) # stepAIC doesn't work for this..
Cary_step<-step(fullish_lmer); summary(get_model(Cary_step))
#  ...>> need to force precip and its square...
Cary_step2<-step(fullish_lmer, keep=attr(terms(fullish_lmer), "term.labels")[1:3]); summary(get_model(Cary_step2))
...
#oooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo
fullML<-lme(log(Yield0)~SeasPr+AvgTemp+I(SeasPr^2)+I(AvgTemp^2)+ Prec2m+CVPrec+Spell+Spell4 +MaxP
               + CVTempK + DDays +HWDays+MaxT, random= ~1 | ID1,method="ML",
               data=ScaledTS,na.action=na.exclude); summary(fullML); exp(summary(fullML)$coef[[1]])
fullML_stepAIC<-stepAIC(fullML) 
summary(fullML_stepAIC) vif(summary(fullML_stepAIC)
...
summary(get_model(Cary2))   ;extractAIC(get_model(Cary2))

Cary3<-step(full, ddf = c("Satterthwaite"), keep=attr(terms(full), "term.labels")[1:2])
summary(get_model(Cary3))   ;extractAIC(get_model(Cary3))

Cary4<-step(full, ddf = c("Satterthwaite"))
summary(get_model(Cary4))   ;extractAIC(get_model(Cary4))
