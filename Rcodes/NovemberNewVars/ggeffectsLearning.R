...
test2<-ggpredict(KEN11a)
test2



KenTest<-lme(Yield~SeasPr+I(SeasPr^2)+CVPrec+Spell+Spell4
                  +AvgTemp + SDTemp, random= ~1 | ID1,data=ScaledTS,na.action=na.exclude); summary(KenTest); summary(KenTest)$tTable[,1]; ranef(KenTest)

test<-ggeffect(KenTest)
test