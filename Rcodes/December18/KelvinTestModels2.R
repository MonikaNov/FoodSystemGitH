...
KEN11h<-update(KEN11d,.~.,method="ML");extractAIC(KEN11h);anova(KEN11g,KEN11h) 
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
KEN11i<-lme(log(Yield0)~SeasPr+I(SeasPr^2)+CVPrec+Spell+Spell4
            +AvgTemp + CVTempK+I(CVTempK*AvgTemp), random= ~1 | ID1,correlation=corARMA(form = ~ as.numeric(Year)|ID1, p=1,q=1),
            data=ScaledTS,na.action=na.exclude); summary(KEN11i); exp(summary(KEN11i)$coef[[1]])
vif(KEN11i)