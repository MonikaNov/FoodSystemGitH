...
test1<-PrecMAM3
sapply(1:8, function(x) {PrecMAM3[[x]]$ASAL=1; return(PrecMAM3[[x]])} ) 
...
test3<-TempMarSep3
TempMarSep3[1:4]<-lapply( TempMarSep3[1:4], function(x){ x$ASAL=1; return(x)} ) 
TempMarSep3[5:8]<-lapply( TempMarSep3[5:8], function(x){ x$ASAL=0; return(x)} ) 
TempOctMar3[1:4]<-lapply( TempOctMar3[1:4], function(x){ x$ASAL=1; return(x)} ) 
TempOctMar3[5:8]<-lapply( TempOctMar3[5:8], function(x){ x$ASAL=0; return(x)} ) 

#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#now merge ASAL and non-ASAL
PrecCoefVar<-rbind(PrecMAM3[[1]],PrecMAM3[[9]])
DrSpell10<-rbind(PrecMAM3[[2]],PrecMAM3[[10]])
DrSpell20<-rbind(PrecMAM3[[3]],PrecMAM3[[11]])
DrySpell<-rbind(PrecMAM3[[4]],PrecMAM3[[12]])
MaxRain<-rbind(PrecMAM3[[5]],PrecMAM3[[13]])
SeasRain<-rbind(PrecMAM3[[6]],PrecMAM3[[14]])
PrecStDev<-rbind(PrecMAM3[[7]],PrecMAM3[[15]])
Prec2Months<-rbind(PrecMAM3[[8]],PrecMAM3[[16]])
...
rm(PrecMAM5)
PrecMAM5<-   mapply (   FUN=function(  x,y)    { names(x)[5] =y;return(x)}   ,x=PrecMAM4,y= names,SIMPLIFY = FALSE )

#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

PrecOND4<-lapply(2:8,  function(x)    rbind(PrecOND3[[x]],PrecOND3[[x+8]])   )
names2<-c("DrSpell10_OND","DrSpell20_OND","DrySpell_OND","MaxRain_OND","SeasRain_OND","PrecStDev_OND","Prec2Months_OND")


PrecOND4<-   lapply (PrecOND4, function(x){ names(x)[4] ="Year";return(x)} )
rm(PrecOND5)
PrecOND5<-   mapply (   FUN=function(  x,y)    { names(x)[5] =y;return(x)}   ,x=PrecOND4,y= names2,SIMPLIFY = FALSE )

#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

TempMarSep4<-lapply(1:4,  function(x)    rbind(TempMarSep3[[x]],TempMarSep3[[x+4]])   )
names3<-c("MaxTemp_MarSep","AvgTemp_MarSep","CumDD_MarSep","HeatWDays_MarSep")


TempMarSep4<-   lapply (TempMarSep4, function(x){ names(x)[4] ="Year";return(x)} )
rm(TempMarSep5)
TempMarSep5<-   mapply (   FUN=function(  x,y)    { names(x)[5] =y;return(x)}   ,x=TempMarSep4,y= names3,SIMPLIFY = FALSE )

#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

TempOctMar4<-lapply(1:4,  function(x)    rbind(TempOctMar3[[x]],TempOctMar3[[x+4]])   )
names4<-c("MaxTemp_OctMar","AvgTemp_OctMar","CumDD_OctMar","HeatWDays_OctMar")


TempOctMar4<-   lapply (TempOctMar4, function(x){ names(x)[4] ="Year";return(x)} )
rm(TempOctMar5)
TempOctMar5<-   mapply (   FUN=function(  x,y)    { names(x)[5] =y;return(x)}   ,x=TempOctMar4,y= names4,SIMPLIFY = FALSE )
