...
#----------------------------------------------------------------------------------------
plot1<-plot(AllPrec)+ylab("Yield")+xlab(NULL)+xlab(NULL)+ggtitle(NULL)+scale_x_continuous(breaks = c(-2,-1,0,1,2,3,4,5)) +theme(axis.title.y=element_text(size=16,face="bold"))
plot2<-plot(AllTemp)+ylab("Yield")+xlab(NULL)+xlab("All")+ggtitle(NULL)+ theme(axis.title=element_text(size=16,face="bold"))
        
plot3<-plot(ASALPrec)+ylab(NULL)+xlab(NULL)+xlab(NULL)+scale_x_continuous(breaks = c(-2,-1,0,1,2,3,4,5))+ggtitle("Seasonal precipitation")+theme(plot.title = element_text(size = 19, face = "bold"))
plot4<-plot(ASALTemp)+ylab(NULL)+xlab(NULL)+xlab("ASAL")+ggtitle("Average temperature")+theme(plot.title = element_text(size = 19, face = "bold"))+theme(axis.title=element_text(size=16,face="bold"))
plot5<-plot(nonASALPrec)+ylab(NULL)+xlab(NULL)+xlab(NULL)+ggtitle(NULL)
plot6<-plot(nonASALTemp)+ylab(NULL)+xlab(NULL)+xlab("non-ASAL")+ggtitle(NULL)+scale_x_continuous(breaks = c(-2,-1.5,-1,-0.5,0,0.5))+theme(axis.title=element_text(size=16,face="bold"))

require(gridExtra)
png("presentations/January2019/Figure1a_1f.png",width = 950, height = 550)
grid.arrange(plot1, plot3,  plot5,plot2,plot4, plot6,ncol=3)
dev.off()
...
require(gridExtra)
png("presentations/January2019/Figure2a_2f.png")
...
require(gridExtra)
pdf("presentations/January2019/Figure3a_3f.pdf")
